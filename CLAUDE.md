...
- **⚠️ Installed on iOS, every height unit lies — `dvh` included (fixed 2026-09-20; the 2026-09-17 attempt did not work).** In a home-screen web app with `black-translucent` + `viewport-fit=cover`, WebKit lays the page out from y=0 — correctly full-bleed under the Dynamic Island — but sizes the **initial containing block from the unobscured content rect: screen MINUS the status bar**. `height: 100%`, `100vh` and **`100dvh` all resolve against it**, so they are all `safe-area-inset-top` (59px on a 15 Pro) short, and a shell that can't scroll has no content flow to push past it. The first fix reached for `dvh` and, believing `position: fixed` obeyed the same short box, added `.mobile-root { transform: translateZ(0) }` to re-anchor the phone's fixed chrome to the app box. Both premises were wrong, and the second one made it worse: the tab bar had been landing on the real screen edge and the transform dragged it 59px up, leaving body's `--bg` showing beneath it (Nic's screenshot, 2026-09-20 — the tab bar's bottom measured exactly 177 device px = 59pt above the screen edge on a 15 Pro).
  ...
  - **Round three (2026-09-20, same evening): `#root { position: fixed }` moved the screenshot by zero pixels** — the tab bar's bottom measured 2378 of 2556 both before and after. Two things came out of looking again:
    ...
    ⚠️ Everything is gated on `navigator.standalone`, and the result is clamped to the top inset. In Safari the viewport is legitimately short — its URL bar is down there — and the same arithmetic would draw the app *underneath* it. `scripts/viewport-check.mjs` pins that case and the landscape long/short-side pick, which would otherwise "correct" a rotated app by 459px.
  - **Round four (2026-09-20): the correction worked, and then `overflow: hidden` clipped it.** The tab bar's top border landed at exactly 92pt from the screen bottom — traction's number — but nothing painted below 59pt: not the labels, not the bar's own background. Position right, paint cut. **`overflow` on the root element propagates to the VIEWPORT, and the viewport's clip rect is the short ICB**, so the shell was laid out correctly and then trimmed at 793pt. traction has no `overflow: hidden` anywhere on its shell, which is the last structural difference between the two and the reason it never showed this.
    - ⚠️ **Nothing in the shell chain (`html`, `body`, `#root`) may set `overflow: hidden`.** It was never scroll prevention here — `#root` is fixed, so nothing is in flow to scroll — and `body { overscroll-behavior: none }` does the job it was actually wanted for (iOS rubber-banding). The scrollers that genuinely clip, `.galaxy` and `.mobile-list`, keep their own. `scripts/smoke.mjs` asserts the chain is clip-free and that all four tab labels render inside the window.
    - This round also settled the open question from round three: the app box being correct **only** with `.ios-short-viewport` applied means the layout viewport really is short, and removing `user-scalable=no` did not by itself fix it. The measurement is load-bearing; keep it.
  - **Round five (2026-09-20): the readings are on screen now — Browse → 📐 Diagnostics.** `DiagnosticsPanel` (AppChrome, own `diag-` namespace) shows the **build stamp** (`__BUILD_ID__`, `define`d in vite.config.ts from the git SHA + build time) and every reading this bug turns on: `navigator.standalone`, `screen`, `clientHeight`, `innerHeight`, `visualViewport`, both insets, the measured shortfall, whether the class applied, the shell's computed `overflow`, and the live rects of `#root` / `.mobile-app` / `.mobile-tabbar` / the lowest tab label.
    ⚠️ **The build stamp is the point.** "The fix did not work" and "the phone never loaded the fix" are indistinguishable from a screenshot, and an installed iOS web app that *resumes* rather than cold-launches never re-navigates — so it can hold old CSS for days and look exactly like a failed fix. Four rounds were argued without being able to tell those apart. Check the stamp before theorising about anything else.
    Mobile-only entry point on purpose: unlike backups, these numbers describe the device in your hand, so "open it on whichever machine is nearest" is not a property worth having. The panel lives in shared chrome so desktop can offer it the day that changes.
  - **Round six — THE ACTUAL CAUSE, from the diagnostics panel's own readings.** On Nic's 15 Pro: `screen` **852**, but `innerHeight` = `visualViewport.height` = `documentElement.clientHeight` = **793**; `overflow` visible throughout; `#root` and `.mobile-app` reporting `0 → 852` and `.mobile-tabbar` `759 → 852`, while the screenshot painted **nothing past 793**.
    - **The web view is smaller than the screen, and you cannot paint outside your own web view.** `apple-mobile-web-app-status-bar-style: black-translucent` makes iOS anchor a standalone web view at y=0 — correctly full-bleed under the Dynamic Island — and size it screen MINUS the status bar. The bottom 59pt belongs to the app window; the flat colour there is the manifest's `background_color`, which is why it matched `--bg` exactly and read as "the app is short".
    - **Every CSS round was therefore unwinnable**, and round three's correction was actively harmful: extending the app box to 852 moved the tab bar's labels OUT of the drawable area. They measured as present (`lowest tab label: 808 of 793`) and never rendered. That is the tell to remember — **a DOM rect past `innerHeight` on iOS standalone is off the web view, not just off-screen.**
    - **The fix is one meta tag: `apple-mobile-web-app-status-bar-style` is now `black`.** iOS then places the web view BELOW the status bar (y=59, height 793, bottom 852 — the real screen edge). Cost: no paint under the Dynamic Island, and the status bar strip goes black, which against `#0a0a1a` is near-invisible. `viewport-fit=cover` stays for the home indicator and landscape insets. It also makes `env(safe-area-inset-top)` 0, so every `--safe-t` offset collapses on its own and `shortfallPx` self-disables (it requires `topInset > 0`).
    - ⚠️ **iOS reads `apple-mobile-web-app-*` when the icon is added to the Home Screen — CONFIRMED, not suspected.** The build after the meta change reported `build: 84d0f81` (so the new HTML was loaded) alongside `safe-area top: 59`, and the screenshot still showed the nebula at y=0–59 instead of a black status-bar strip. Under `black` that inset is 0 and the top goes opaque. **The installed app was still running black-translucent from whenever its icon was created**, and no amount of force-quitting re-reads the tag. The icon must be deleted and re-added — once.
      - ⚠️ **Deleting a home-screen PWA deletes its storage sandbox**, which is the exact shape of the 2026-08-22 data loss (see Sync). Export JSON from Backups & history first, every time, even signed in — undo/redo never schedule a cloud save, so recent undone work can be local-only.
      - **How to tell the re-install took:** Browse → Diagnostics should read `safe-area top: 0`, `measured shortfall: 0px`, `correction applied: false`, and `.mobile-tabbar rect` ending at the same number as `clientHeight`.
