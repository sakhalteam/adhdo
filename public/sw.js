/*
 * adhdo's service worker — its whole job is making the app open with no signal.
 *
 * This one earns its keep on a mountain pass. The state layer is already
 * localStorage-first and syncs to Supabase on a debounce, so thoughts captured
 * out of range are safe; what was missing was the app *shell*. Without a worker,
 * no bars means a blank page, and a thought you can't write down is a thought
 * you lose.
 *
 ...
 *                     correct (and instant).
 *   - anything else → straight to the network, uncached. Supabase reads and
 *                     writes must never be answered from a cache.
 */

const CACHE = 'adhdo-v1'
...
/** Icons and the manifest: same-origin, rarely change, safe to serve from cache. */
